...
class UnetScratch(nn.Module):
    ...
    def __init__(self, in_channels: int = 3, num_classes: int = 1, num_channels: list = (64, 128, 256, 512)):
        ...
        # self.encoder = nn.ModuleList([
        #     DoubleConv(in_channels, num_channels[0]),
        #     DoubleConv(num_channels[0], num_channels[1]),
        #     DoubleConv(num_channels[1], num_channels[2]),
        #     DoubleConv(num_channels[2], num_channels[3]),
        #     DoubleConv(num_channels[3], num_channels[4])
        #     ])
        ...
        for channel_num in reversed(num_channels):
            ...
            self.decoder.append(nn.ConvTranspose2d(in_channels=channel_num*2, out_channels=channel_num, kernel_size=2, stride=2))
        ...
        self.network_in_network = nn.Conv2d(in_channels=num_channels[0], out_channels=num_classes, kernel_size=1, padding=0)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        args:
            1. x: A channels_first Torch 4D-Tensor of shape (m, C, H, W)
        '''
        # a list to store skip connections' input (from encoder part)
        skip_connections = []
        ...
        for i, step in enumerate(self.decoder):
            ...
            if i % 2 == 0:
                ...
                if x.shape != skip_connection.shape:
                    ...
                    # method 3: use resizing
                    skip_connections_cropped = TF.Resize(x.shape[2:])(skip_connection)
